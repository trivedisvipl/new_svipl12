...
class CareersDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
            ...
            //     $photo = CLIENT_IMAGE;
            //     if($data->image){
            //         if (File::exists(CLIENT_IMAGE_PATH.$data->image) ) {
            //             $photo = CLIENT_IMAGE_ROOT.$data->image;
            //         }
            //     }
            //     return '<img src="'.$photo.'" alt="Client img" class="datatable-img">';
            // })
            ...
            ->addColumn('action', function($data){
                $html = '<button class="btn btn-danger m-1" onclick="deleteData('.$data->id.')"><i class="fa fa-trash"></i></button>';
                   $html .= '<a href="#"  onclick="viewData(this)" data-info=\''.json_encode($data).'\' class="btn btn-primary m-1"><i class="fa fa-eye"></i></a>';
                // $html .= '<a href="'.route("clients.edit",  $data->id).'" class="btn btn-primary m-1"><i class="fa fa-edit"></i></a>';
                return $html;
    ...
    public function getColumns(): array
    {
        return [
            ...
            // Column::make('experience')->addClass('text-center'),
            // Column::make('skills')->addClass('text-center'),
            // Column::make('notice_period')->addClass('text-center'),
            // Column::make('resume')->addClass('text-center'),
