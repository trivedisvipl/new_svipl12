...
class ExpertiseDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
            ...
            ->addColumn('image', function($data){
                $photo = EXPERTIES_IMAGE;
                if($data->image){
                    if (File::exists(EXPERTIES_IMAGE_PATH.$data->image) ) {

                        $photo = EXPERTIES_IMAGE_ROOT.$data->image;
                    }
                }
                return '<img src="'.$photo.'" alt="Expertise img" class="datatable-img">';
            })
            ...
            ->addColumn('action', function($data){
                ...
                $html .= '<a href="'.route("experties.edit",  $data->id).'" class="btn btn-primary m-1"><i class="fa fa-edit"></i></a>';
                return $html;
            })->setRowId('id')->rawColumns([ 'image','status', 'action']);
     ...
     */
    public function query(Expertise $model): QueryBuilder
    {
